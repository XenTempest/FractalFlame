...
class NonLinearFunction {
    ...
    constructor(name, implementation, paramsRadii = []) {
        ...
        if (paramsRadii.length) {
            this.implementation = implementation(this.params);
        }
        else {
...
function toStep(f) {
    return /* is3D ? (position: Vector3D) => new Vector3D(f(position.x), f(position.y), f(position.z)) : */ (position) => new Vector2D(f(position.x), f(position.y));
}
function sinRandom() {
